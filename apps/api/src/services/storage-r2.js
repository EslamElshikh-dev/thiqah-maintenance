...
async function deleteRejectedObject(client, bucket, key) {
  try {
...
async function inspectObject(client, bucket, key, maxSizeBytes) {
  const response = await client.send(new GetObjectCommand({ Bucket: bucket, Key: key }));
  if (!response.Body) throw new Error('UPLOAD_NOT_FOUND');
  if (Number(response.ContentLength || 0) > maxSizeBytes) throw new Error('FILE_TOO_LARGE');
...
export function createR2StorageService(config) {
  ...
  return {
    ...
    async createReadUrl({ db, actor, orderId, mediaId }) {
      ...
      const readUrl = await getSignedUrl(client, command, { expiresIn: 5 * 60 });
      return {
    ...
    async completeUpload({ db, actor, intentId, sha256Hex }) {
      return db.tx(async (dbClient) => {
        ...
        let inspected;
        try {
          const head = await client.send(new HeadObjectCommand({ Bucket: bucket, Key: intent.object_key }));
          const metadataSize = Number(head.ContentLength || 0);
          if (!metadataSize || metadataSize > Number(intent.max_size_bytes)) throw new Error('FILE_TOO_LARGE');
          inspected = await inspectObject(client, bucket, intent.object_key, Number(intent.max_size_bytes));
        } catch (error) {
          if (isNotFound(error)) throw badRequest('UPLOAD_NOT_FOUND', 'Uploaded object was not found');
          await deleteRejectedObject(client, bucket, intent.object_key);
