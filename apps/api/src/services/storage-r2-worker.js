...
export function signR2WorkerUrl(config, method, key, expiresInSeconds, options = {}) {
  const exp = Math.floor(Date.now() / 1000) + expiresInSeconds;
  const contentType = options.contentType || '';
  const disposition = options.disposition || '';
  const size = options.size === undefined ? '' : String(options.size);
  const signature = createHmac('sha256', config.r2WorkerHmacKey)
    .update(canonical(method, key, exp, contentType, disposition, size))
    .digest('base64url');
  const url = new URL(`${config.r2WorkerUrl}/v1/object/${encodeURIComponent(key)}`);
  url.searchParams.set('exp', String(exp));
  url.searchParams.set('sig', signature);
  if (contentType) url.searchParams.set('ct', contentType);
  if (disposition) url.searchParams.set('disp', disposition);
  if (size !== '') url.searchParams.set('size', size);
  return { url: url.toString(), expiresAt: new Date(exp * 1000) };
}

async function brokerFetch(config, method, key, options = {}) {
  const signed = signR2WorkerUrl(config, method, key, options.expiresInSeconds || 120, options);
...
async function deleteRejectedObject(config, key) {
  try {
    await brokerFetch(config, 'DELETE', key);
  } catch {
    // Preserve the validation error as the primary failure.
  }
}

async function inspectObject(config, key, maxSizeBytes) {
  const response = await brokerFetch(config, 'GET', key, { timeoutMs: 30_000 });
  if (response.status === 404) throw new Error('UPLOAD_NOT_FOUND');
  if (!response.ok || !response.body) throw new Error('UPLOAD_READ_FAILED');
  const advertised = Number(response.headers.get('content-length') || 0);
  if (advertised > maxSizeBytes) throw new Error('FILE_TOO_LARGE');
  ...
  for await (const chunkValue of response.body) {
    ...
    if (sizeBytes > maxSizeBytes) throw new Error('FILE_TOO_LARGE');
    hash.update(chunk);
    if (headerBytes < 16) {
      const slice = chunk.subarray(0, 16 - headerBytes);
...
export function createR2WorkerStorageService(config) {
  return {
    kind: 'r2-worker',
    status: 'ready',
    ...
    async createUploadIntent({ db, actor, orderId, kind, mimeType, sizeBytes }) {
      ...
      const signed = signR2WorkerUrl(config, 'PUT', objectKey, 10 * 60, { contentType: mimeType, size: sizeBytes });
      return {
        intentId: intent.rows[0].id,
        objectKey,
        uploadUrl: signed.url,
        requiredHeaders: Object.freeze({ 'content-type': mimeType, 'if-none-match': '*' }),
    ...
    async createReadUrl({ db, actor, orderId, mediaId }) {
      ...
      const signed = signR2WorkerUrl(config, 'GET', media.object_key, 5 * 60, {
    ...
    async completeUpload({ db, actor, intentId, sha256Hex }) {
      return db.tx(async (dbClient) => {
        const selected = await dbClient.query(`SELECT * FROM thiqah.media_upload_intents WHERE id=$1 FOR UPDATE`, [intentId]);
        ...
        try {
          const head = await brokerFetch(config, 'HEAD', intent.object_key);
          if (head.status === 404) throw new Error('UPLOAD_NOT_FOUND');
          if (!head.ok) throw new Error('UPLOAD_HEAD_FAILED');
          const metadataSize = Number(head.headers.get('content-length') || 0);
          if (!metadataSize || metadataSize > Number(intent.max_size_bytes)) throw new Error('FILE_TOO_LARGE');
        } catch (error) {
          if (error?.message === 'UPLOAD_NOT_FOUND') throw badRequest('UPLOAD_NOT_FOUND', 'Uploaded object was not found');
          await deleteRejectedObject(config, intent.object_key);
          throw badRequest('UPLOAD_VALIDATION_FAILED', 'Uploaded file failed server-side validation');
        }

        let inspected;
        try {
          inspected = await inspectObject(config, intent.object_key, Number(intent.max_size_bytes));
        } catch (error) {
          if (error?.message === 'UPLOAD_NOT_FOUND') throw badRequest('UPLOAD_NOT_FOUND', 'Uploaded object was not found');
          await deleteRejectedObject(config, intent.object_key);
          throw badRequest('UPLOAD_VALIDATION_FAILED', 'Uploaded file failed server-side validation');
        ...
        if (!inspected.sizeBytes || inspected.sizeBytes > Number(intent.max_size_bytes) || inspected.mimeType !== intent.mime_type || inspected.sha256Hex !== sha256Hex) {
          await deleteRejectedObject(config, intent.object_key);
        ...
        const media = await dbClient.query(
        ...
        await dbClient.query(`UPDATE thiqah.media_upload_intents SET completed_at=now() WHERE id=$1`, [intent.id]);
        return media.rows[0];
      });
    }
  };
